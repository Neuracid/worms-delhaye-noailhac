...
bool Commandes::deplacementDroite(Etat &etat){
  ...
  if(y>=0 && y<etat.grille.getLargeur()-1){
    int type=etat.listeBloc.matriceTerrain[x][y+1]->getType();
    switch (type) {
      case 0: etat.listeWormsJoueurs[etat.placeWormsActif].worms->setPosition_y(y+1);verif=true;break; //air
      case 1: etat.listeWormsJoueurs[etat.placeWormsActif].worms->setPosition_y(y+1);verif=true;break; //gaz
      case 2: etat.listeWormsJoueurs[etat.placeWormsActif].worms->setPosition_y(y+1);verif=true;break; //eau
      default: //terre ou roche ou metal
              if(x>0 && x<etat.grille.getHauteur()){
...
bool Commandes::deplacementGauche(Etat &etat){
  ...
  int x=etat.listeWormsJoueurs[etat.placeWormsActif].worms->getPosition_x();
  int y=etat.listeWormsJoueurs[etat.placeWormsActif].worms->getPosition_y();
  if(y>0 && y<etat.grille.getLargeur()){
    int type=etat.listeBloc.matriceTerrain[x][y-1]->getType();
    switch (type) {
      case 0: etat.listeWormsJoueurs[etat.placeWormsActif].worms->setPosition_y(y-1);verif=true;break; //air
      case 1: etat.listeWormsJoueurs[etat.placeWormsActif].worms->setPosition_y(y-1);verif=true;break; //gaz
